...
function main() {
  ...
  const pkgName = flags.official
    ? `@sfmc-bds/module-${folderId}`
    : `@${scope}/sfmc-module-${folderId}`;
  ...
  replace("sapi/src/index.ts", (s) => {
    ...
    r = r.replaceAll(`"${PLACEHOLDER_DISPLAY}"`, `"${displayName}"`);
    r = r.replaceAll(`@sfmc-bds/module-${PLACEHOLDER_ID}`, pkgName);
    r = r.replaceAll(`@CHANGE_ME/sfmc-module-${PLACEHOLDER_ID}`, pkgName);
    return r;
  });
  ...
  if (fs.existsSync(oldTest)) {
    ...
    body = body.replaceAll(PLACEHOLDER_ID, folderId).replaceAll(PLACEHOLDER_LOGICAL, logicalId);
